...
grammar::grammar() {
  rules['#'].push_back(
      rule('#', {'S'})); // # - это S', добавили переход S' -> S
...
LR::action &LR::action::operator=(
    const action &another) { // при попытке перезаписать ячейку таблицы -
                             // выбрасывается исключение
  if (act != '-' &&
      (this->act != another.act || this->number != another.number))
    throw bad_grammar();
  act = another.act;
...
LR::situation::situation(char from, const deque<char> &left,
                         const deque<char> &right, char next)
...
void LR::First() {
  ...
  for (auto c : G.alphabet) {
    map<char, bool> used;
    set<char> ans;
    dfs(c, Graph, used, ans,
        G.term); // ищем все достижимые терминалы из символа c - FIRST(c)
    first[c] = ans;
  }
}
